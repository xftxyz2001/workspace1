...
def get_folder_classify_info(folder_path, extension):
    """更新并返回指定目录下的文件信息
...
def update_classify_info(folder_path, df):
    """更新文件信息

    Args:
        folder_path (str): 路径
        df (pd.DataFrame): 文件信息
    """
    df.to_csv(os.path.join(folder_path, classify_info_file), index=False)


if __name__ == "__main__":
    folder_path = "."
    extension = "*.py"
    df = get_folder_classify_info(folder_path, extension)
    print(df)
    # 修改v1.py的分类状态
    df.loc[df["文件名"] == "v1.py", "分类状态"] = "已分类"
    df.loc[df["文件名"] == "v1.py", "类别"] = "Python"
    update_classify_info(folder_path, df)
